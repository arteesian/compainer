...
document.addEventListener('DOMContentLoaded', () => {
  ...
  searchForm?.addEventListener('submit', (e) => {
    ...
    wrap.innerHTML = `
        ...
        <button class="ga-modal-copy-vip hidden">Скопировать</button></div>
        </div>
    `;
    ...
    wrap.querySelector('.ga-modal-copy-vip').addEventListener('click', async () => {
        const text = wrap.querySelector('.ga-modal-content').innerText;
        try {
            await navigator.clipboard.writeText(text);
            // мини-обратная связь
            const btn = wrap.querySelector('.ga-modal-copy-vip');
    ...
    document.addEventListener('click', async (ev) => {
      const btn = ev.target.closest('.ga-acc-copy-vip');
      if (!btn) return;
    ...
    document.addEventListener('click', async (e) => {
      ...
      const addFaqBtn = e.target.closest('#button-add-faq-vip');
      if (addFaqBtn) {
      ...
      const submitBtn = e.target.closest('.ga-btn-submit-vip');
      if (submitBtn) {
        const overlay = document.querySelector('.ga-modal-overlay');
    ...
    function openModal({ text = '', withCopy = false }) {
        ...
        const copyBtn = overlay.querySelector('.ga-modal-copy-vip');
    ...
    function buildFaqAccordionHtml(list) {
      ...
      const addBtn = `<div class="ga-faq-add-wrap">
        <button type="button" id="button-add-faq-vip">+ Добавить частый вопрос</button>
      </div>`;
      ...
      const items = approved.map((qa) => {
        ...
        return `
          <details class="ga-acc-item">
            <summary class="ga-acc-summary-vip"><span class="ga-acc-q">${q}</span></summary>
            <div class="ga-acc-content">
              <div class="ga-acc-answer" data-answer="${a}">${a}</div>
              <div class="ga-acc-copy-block"><button type="button" class="ga-acc-copy-vip">Скопировать</button></div>
            </div>
          </details>
        `;
      }).join('');
      ...
      content.innerHTML = `
        <div class="ga-faq-form">
          ...
          <div class="ga-form-actions">
            ...
            <button type="button" class="ga-btn ga-btn-submit-vip">Отправить на согласование</button>
          </div>
        </div>
      `;
  ...
  function render(actions) {
    ...
    if (topbarCountEl) topbarCountEl.innerHTML = `<span class="pagination-general-for-vip">${start}-${end}</span> из ${end}`;
